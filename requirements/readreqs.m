...
arguments ( Input )
    textRequirementsFiles(1, :) string {mustBeFile} = ...
        projectTextRequirements()
end % arguments ( Input )
...
if ~isempty( proj )
    ...
    slreqxFileNoRoot = replace( slreqxFile, projectRoot, "." );
else
    slreqxFileNoRoot = slreqxFile;
end % if
...
for linkSetIdx = 1 : numel( linkSets )
    ...
    fprintf( "[+] %s\n", replace( linkSets(linkSetIdx).Filename, ...
        projectRoot, "." ) )
end % for
